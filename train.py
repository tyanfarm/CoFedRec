...
import argparse
from mlp_supcon import MLPEngine
...
parser.add_argument('--alias', type=str, default='CoFedRec')
...
parser.add_argument('--user_cluster', type=int, default=1)
parser.add_argument('--item_cluster', type=int, default=30)
...
parser.add_argument('--neighborhood_size', type=float, default=0.0)
parser.add_argument('--neighborhood_threshold', type=float, default=0.5)
parser.add_argument('--reg_item', type=float, default=0.5)
...
str = current_time + '-' + 'latent_dim: ' + str(config['latent_dim']) + '-' + 'lr: ' + str(config['lr']) + '-' + \
      ...
      'optimizer: ' + config['optimizer'] + '-' + 'l2_regularization: ' + str(config['l2_regularization']) + ' - ' + 'reg: ' + str(config['reg']) + ' - ' + 'cl_t: ' + str(config['cl_t']) + ' - ' + 'method: ' + config['alias']
file_name = "sh_result/"+config['dataset']+".txt"
with open(file_name, 'a') as file:
    file.write(str + '\n')

# Save results to discord
WEBHOOK_URL = "https://discord.com/api/webhooks/1378094332355154025/l7_qy_YVfPTg4WSmdd5Hk5f9ix81hLbhAv4YACosLQMt_r8GWpEnUD6dHRvXd4Qf2SqU"
